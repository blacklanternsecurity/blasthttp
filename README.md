...
A response whose body won't decode is still returned: the status line and headers arrived cleanly, and losing the response looks the same to the caller as an unreachable host. When that happens `content` holds the bytes exactly as the server sent them, still encoded, and `decode_error` says why. Anything that hashes, matches, or diffs bodies should check it, since compressed bytes are otherwise indistinguishable from content.
